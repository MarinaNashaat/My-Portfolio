$(document).ready(function() {
    $(window).scroll(function() {
        ...
        if ($(this).scrollTop() > 100) {
            $('.nav-header').addClass('active').removeClass('inactive');
        } else {
            $('.nav-header').removeClass('active').addClass('inactive');
        }
    });
...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        ...
        $('.navbar-link a').on('click', function() {
            $('.navbar-link a').removeClass('activebar');
            $(this).addClass('activebar');
        });
    });
...
$(document).ready(function() { // Tells the f//unction to wait to preform until everything on the page has loaded.
    $(window).scroll(function() { // Says this function is preformed continuisly while scrolling.
        var Scroll = $(window).scrollTop(), // This variable finds the distance you have scrolled from the top.
            ...
            SectionTwoOffset = $('#projects').offset().top; // This variable finds the distance between #section-two and the top. Replace #section-two with the ID of your section. You can duplicate this for as many sections as you want.
        ...
        if (Scroll >= SectionOneOffset) { // If you have scrolled past section one do this.
            $(".menu-item-1").addClass("activebar"); // Adds class of current-menu-item to the menu item with a class of menu-item-1
        } else { // If you have not scrolled section one do this.
            $(".menu-item-1").removeClass("activebar"); // Removes class of current-menu-item to the menu item with a class of menu-item-1
        }
